...
async function mergeOpenAPIFiles(folderPath) {
  ...
  for (const fileName of fileNames) {
    ...
    Array.prototype.push.apply(mergedOpenAPI.tags, openAPIObject.tags);
  ...
  if (Object.keys(mergedOpenAPI.paths).length == 0) {
    throw Error(
      "Please make sure you have atleast one valid openAPI specification file in json format under iven folder location."
    );
  }
